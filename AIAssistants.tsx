...
const ChatAssistant: React.FC<{ onClose: () => void; establecimiento: string; }> = ({ onClose, establecimiento }) => {
  ...
  return (
    <div className="fixed bottom-24 right-4 w-[400px] h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col border border-slate-200 animate-in fade-in slide-in-from-bottom-4 duration-300">
      <header className="p-4 border-b flex justify-between items-center bg-slate-50 rounded-t-2xl">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-bold text-slate-800">Asistente Experto</h3>
        </div>
    ...
    return (
     <div className="fixed bottom-24 right-4 w-[400px] h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col border border-slate-200 animate-in fade-in slide-in-from-bottom-4 duration-300">
        <header className="p-4 border-b flex justify-between items-center bg-slate-50 rounded-t-2xl">
            ...
            <h3 className="font-bold text-slate-800">Asistente de Voz</h3>
            </div>
            <button onClick={() => { stopConversation(); onClose(); }} className="text-slate-400 hover:text-slate-600"><LucideIcons.X size={20}/></button>
        </header>
        <div className="flex-grow p-4 overflow-y-auto bg-slate-100 flex flex-col-reverse">
        ...
        <footer className="p-4 border-t flex flex-col items-center justify-center">
            {status === 'idle' ? (
                <button onClick={startConversation} className="p-4 bg-green-500 text-white rounded-full shadow-lg hover:bg-green-600 transition">
                    <LucideIcons.Mic size={28}/>
                </button>
            ) : (
                <button onClick={stopConversation} className="p-4 bg-red-500 text-white rounded-full shadow-lg hover:bg-red-600 transition">
                    <LucideIcons.Square size={28}/>
                </button>
            )}
             <p className="text-xs text-slate-500 mt-2 font-semibold">
                {status === 'idle' ? 'Presiona para hablar' : status === 'listening' ? 'Escuchando...' : status === 'speaking' ? 'Hablando...' : 'Procesando...'}
            </p>
        </footer>
     </div>
    );
...
export const AIAssistantSuite: React.FC<{ establecimiento: string }> = ({ establecimiento }) => {
  const [openAssistant, setOpenAssistant] = useState<'chat' | 'voice' | null>(null);

  const toggleAssistant = (assistant: 'chat' | 'voice') => {
    setOpenAssistant(prev => (prev === assistant ? null : assistant));
  };
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-3">
       ...
       <div className="flex flex-col items-end gap-3">
            <button onClick={() => toggleAssistant('voice')} title="Asistente de Voz"
                className={`p-4 bg-white rounded-full shadow-lg border transition-all duration-300 ${openAssistant === 'voice' ? 'bg-red-500 text-white' : 'text-red-500 hover:bg-red-50'}`}>
                <LucideIcons.Mic size={24}/>
            </button>
            <button onClick={() => toggleAssistant('chat')} title="Asistente de Chat"
                className={`p-4 bg-white rounded-full shadow-lg border transition-all duration-300 ${openAssistant === 'chat' ? 'bg-blue-600 text-white' : 'text-blue-600 hover:bg-blue-50'}`}>
